...
(defun correlate-human-model-performance (grid-side
  ...
  (dlet* (((human-tracking-performance human-id-performance)
    ...
    (corrcoef human-tracking-performance
...
(defun parameter-heatmap (&optional participant-id)
  (let* ((parameter-file
           (if participant-id
               (format
                nil
                "~~/quicklisp/local-projects/tracking-without-indices/parameter-search-floc-mse.~A.txt"
                participant-id)
               "~/quicklisp/local-projects/tracking-without-indices/parameter-search-floc-mse.txt"))
    ...
    (let ((floc-optim (loop :for nob :below 120
                            :collect (* nob
